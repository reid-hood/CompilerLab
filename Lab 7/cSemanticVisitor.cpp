...
static string TypeName(cDeclNode *type)
{
    ...
    if (type->IsStruct()) return type->GetName();
    return "unknown";
}
...
cDeclNode *cSemanticVisitor::InferVarType(cVarExprNode *node)
{
    ...
    for (int i = 1; i < node->GetPartCount(); i++)
    {
        ...
        if (fieldSym != nullptr)
        {
            ...
            if (container->IsVar()) container = container->GetType();
            ...
            if (fields != nullptr)
            {
                for (int j = 0; j < fields->GetDeclCount(); j++)
                {
                    ...
                    if (field->GetName() == fieldSym->GetName())
                    {
                        matched = field;
                        break;
                    }
...
void cSemanticVisitor::Visit(cVarExprNode *node)
{
    ...
    string currentName = base->GetName();
    for (int i = 1; i < node->GetPartCount(); i++)
    {
        cAstNode *part = node->GetPart(i);
        ...
        if (fieldSym != nullptr)
        {
            ...
            {
                node->SemanticError(currentName + " is not a struct");
                break;
            }
            ...
            if (matched == nullptr)
            {
                node->SemanticError(fieldSym->GetName() + " is not a field of " + currentName);
                break;
            }

            // Canonicalize the field symbol in the varref so emitted XML uses
            // the same symbol identity as the field declaration.
            cVarDeclNode *fieldVarDecl = dynamic_cast<cVarDeclNode*>(matched);
            if (fieldVarDecl != nullptr && fieldVarDecl->GetNameSym() != nullptr)
            {
                node->SetPart(i, fieldVarDecl->GetNameSym());
                fieldSym = fieldVarDecl->GetNameSym();
            }

            current = matched;
            currentName += "." + fieldSym->GetName();
            continue;
        }
        ...
        {
            cVarDeclNode *varDecl = dynamic_cast<cVarDeclNode*>(arrayDecl);
            if (varDecl == nullptr || !varDecl->IsArray())
            {
                node->SemanticError(currentName + " is not an array");
                break;
            }
        ...
        {
            node->SemanticError("Index of " + currentName + " is not an int");
        }
...
void cSemanticVisitor::Visit(cFuncCallNode *node)
{
    ...
    {
        node->SemanticError(funcSym->GetName() + " is not a function");
        return;
    }
